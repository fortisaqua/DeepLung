...
parser.add_argument('--save-freq', default='10', type=int, metavar='S',
                    help='save frequency')
parser.add_argument('--resume', default='results/dpn3d26/retrft960/200.ckpt', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--save-dir', default='dpn3d26/retrft960/', type=str, metavar='SAVE',
                    help='directory to save checkpoint (default: none)')
parser.add_argument('--test', default=0, type=int, metavar='TEST',
                    help='1 do test evaluation, 0 not')
parser.add_argument('--testthresh', default=0, type=float,
...
def main():
    ...
    logfile = os.path.join(save_dir,'log_8hard_temp')
    if args.test!=1:
...
def test(data_loader, net, get_pbb, save_dir, config, sidelen):
    ...
    for i_name, (data, target, coord, nzhw) in enumerate(data_loader):
        ...
        tp, fp, fn , lbbCount = acc(pbb,lbb,0,0.1,0.1)
        print([len(tp), len(fp), len(fn), lbbCount])
        print([i_name,name])
        totalFP += len(fp)
        totalFN += len(fn)
        totalTP += len(tp)
    ...
    print('got total true positive bbx: ',totalTP)
    print('got total false positive bbx: ',totalFP)
    print('got total false negative bbx: ',totalFN)
    print('label bounding box amount: ',totalLB)
    print
    print
